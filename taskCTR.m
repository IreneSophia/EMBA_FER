% This function presents the animal recognition control (CTR) task from the
...
% emotionally neutral face gradually and continually transform into an 
% animal. The video can be stopped with key_stop as soon as the animal has 
% been recognised. Then, participants can choose the corresponding species 
% from four options by using the key_left and key_right to select an option 
% that they confirm by using the key_choose. There are 16 trials.
...
fid = fopen(path_dat + "\Data\CTR-BV-" + subID + "_" + datestr(datetime(),'yyyymmdd-HHMM') + ".csv", 'w');
fprintf(fid, 'subID,trl,video,animal,disc,opt,lastkey\n');
...
if eyeTracking
    ...
    crsLiveTrackSetDataFilename(char(path_dat + "\Data\CTR-ET-" + subID + "_" + datestr(datetime(),'yyyymmdd-HHMM') + ".csv"));
end
...
try
    ...
    for j = 1:ntrials
        ...
        pic_width  = size(pic,2)*1.3/2; % we want the images to be 1.3 times their normal size
        pic_height = size(pic,1)*1.3/2;
        ...
        while ~strcmp(key,key_choose) && (GetSecs - t_stop < maxrs)
            ...
            CTR_mc(opt, xCenter, yCenter, window, black, grey);
