...
e1
e1_equ = equivalence_test(df.m.fer %>% 
...
p = ggarrange(p1, p2, p3, p4, labels = "AUTO", ncol = 1, nrow = 4)
annotate_figure(p, 
                top = text_grob("Results of SBC including FSR discrimination threshold", 
                face = "bold", size = 14))
...
point.args.pos = list(position = ggpp::position_dodgenudge(x = -0.25, width = 0.1))) +
  ...
  ylim(0, 1) +
  labs(title = "Discrimination threshold per subject", 
       x = "", 
...
# read in the results
df.err.bf = read_csv(file.path(sense_dir, sprintf("df_%s_bf.csv", main.code)), 
                     show_col_types = F)
