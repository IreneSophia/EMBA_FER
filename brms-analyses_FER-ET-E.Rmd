...
p1 = pp_check(m.first, ndraws = nsim, type = 'bars') + 
  ...
  scale_x_continuous(name = "", breaks = c(1, 2, 3, 4), 
                     labels = c("nose", "mouth", "forehead", "eyes"))
...
# read in the results
df.first.bf = read_csv(file.path(sense_dir, sprintf("df_%s_bf.csv", main.code)), 
                       show_col_types = F)
...
df.last %>% group_by(diagnosis, emo) %>% count(AOI.code) %>% 
  ...
  stat_pointinterval(data = df.last.epred,
           ...
           show.legend = FALSE) + 
  scale_fill_manual(values = custom.col, 
                    labels = c("fear", "anger", "happiness", "sadness")) +
  theme_bw() + ylim(0, 1) +
  theme(legend.position = "bottom") + 
  labs(title = "Expectations based on the model on top of real data", y = "percentage") +
...
# read in the results
df.last.bf = read_csv(file.path(sense_dir, sprintf("df_%s_bf.csv", main.code)), 
                      show_col_types = F) %>%
